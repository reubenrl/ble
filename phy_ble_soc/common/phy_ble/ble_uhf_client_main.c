...
#define PHY_BLE_UUID_SERVICE	3335d82a-51f8-4c7c-9bcd-12a4c3d98064
#define PHY_BLE_UUID_CHAR		da9547f0-4a25-4d4c-adbc-77a16520a006

/// test
#define d(s)	(s>>(128-(8*1))),(s>>(128-(8*2))),(s>>(128-(8*3))),(s>>(128-(8*4))),(s>>(128-(8*5))),(s>>(128-(8*6))),		\
				(s>>(128-(8*7))),(s>>(128-(8*8))),(s>>(128-(8*9))),(s>>(128-(8*10))),(s>>(128-(8*11))),(s>>(128-(8*12))),	\
				(s>>(128-(8*13))),(s>>(128-(8*14))),(s>>(128-(8*15))),(s>>(128-(8*16)))
///

const uint8 serviceUUID[UUID128_BUF_LEN] = {0x64,0x80,0xd9,0xc3,0xa4,0x12,0xcd,0x9b,0x7c,0x4c,0xf8,0x51,0x2a,0xd8,0x35,0x33};
const uint8 charUUID[UUID128_BUF_LEN] = {0x06,0xa0,0x20,0x65,0xa1,0x77,0xbc,0xad,0x4c,0x4d,0x25,0x4a,0xf0,0x47,0x95,0xda};
...
static int process_scan_response(struct gecko_msg_le_gap_scan_response_evt_t *pResp)
{
    ...
    while (i < (pResp->data.len - 1))
    {
        ...
        if (ad_type == 0x06 || ad_type == 0x07)
        {
        	...
        	if(memcmp(serviceUUID, &(pResp->data.data[i+2]),UUID128_BUF_LEN) == 0)
        	{
        		printf("Found phy_ble_server device\r\n");
        		ad_match_found = 1;
        	}
        }
...
void ble_uhf_client_main(void)
{
  ...
  while (1) {
    ...
    case gecko_evt_le_connection_opened_id:
    	...
    	gecko_cmd_gatt_discover_primary_services_by_uuid(_conn_handle, UUID128_BUF_LEN, serviceUUID);
    ...
    case gecko_evt_gatt_service_id:
    	...
    	if(evt->data.evt_gatt_service.uuid.len == UUID128_BUF_LEN)
    	{
    		if(memcmp(serviceUUID, evt->data.evt_gatt_service.uuid.data,UUID128_BUF_LEN) == 0)
    		{
    ...
    case gecko_evt_gatt_procedure_completed_id:
    	...
    	case FIND_SERVICE:
    		...
    		else
    		{
    			...
    			printf("phy_ble - service not found?\r\n");
    	...
    	case FIND_CHAR:
    		...
    		else
    		{
    			...
    			printf("phy_ble -  characteristic not found?\r\n");
    			gecko_cmd_endpoint_close(_conn_handle);
    		}
    		break;
    	...
    	case gecko_evt_gatt_characteristic_id:
    		...
    		if(evt->data.evt_gatt_characteristic.uuid.len == UUID128_BUF_LEN)
    		{
    			if(memcmp(charUUID, evt->data.evt_gatt_characteristic.uuid.data,UUID128_BUF_LEN) == 0)
    			{
    	...
    	case gecko_evt_gatt_characteristic_value_id:
    		...
    		if(evt->data.evt_gatt_characteristic_value.characteristic == _char_handle)
    		{
    			...
    			printf("sensor temperature:%lu\r\n", (BYTES_TO_UINT32(printbuf[1],printbuf[2],printbuf[3],printbuf[4]))/1000U );


    			//printf("gatt_characteristic_value.value.len:%d\r\n", evt->data.evt_gatt_characteristic_value.value.len);
    		}
    		break;
