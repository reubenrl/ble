...
#define DELAY_SEC_TIMER		3
#define TIMEOUT_MS_TIMER 	3
#define TEMPERATURE_TIMER	SPP_TX_TIMER
...
void temperatureMeasure()
{
  ...
  if (Si7013_MeasureRHAndTemp(I2C0, SI7021_ADDR, &rhData, &tempData) == 0) {
    ...
    printf("rhData%d:temperature%d", rhData, temperature);

    /* Send indication of the temperature in htmTempBuffer to all "listening" clients.
     * This enables the Health Thermometer in the Blue Gecko app to display the temperature.
     *  0xFF as connection ID will send indications to all connections. */
    gecko_cmd_gatt_server_send_characteristic_notification(_conn_handle, gattdb_temp_measurement, 5, htmTempBuffer);
  }
}
    	...
    	if(pStatus->characteristic == gattdb_gatt_spp_data){
    		if(pStatus->status_flags == gatt_server_client_config){
    			...
    			if(pStatus->client_config_flags == gatt_notification){
    				...
    				gecko_cmd_hardware_set_soft_timer(32768UL*5UL, SPP_TX_TIMER, 0); // 5 seconds
